...
def config():
    ...
    dataroot = '/lfs/1/gangus/data/chexnet'
    # input batch size
    batchSize = 1
...
class Harness:
    ...
    def _init_visualizer(self):
        ...
        # create website
        web_dir = os.path.join(self.opts.results_dir, self.opts.name, '%s_%s' % (
            self.opts.phase, self.opts.which_epoch))
        webpage = html.HTML(web_dir, 'Experiment = %s, Phase = %s, Epoch = %s' % (
            self.opts.name, self.opts.phase, self.opts.which_epoch))
...
#                break
            ...
            if sourceoftest == 'internal':
                ...
                self.visualizer.save_images_nogt(
